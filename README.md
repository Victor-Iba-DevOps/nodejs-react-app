...
*   В рабочую директорию локальной машины командой \
 `git clone git@git.zby.icdc.io:icdc/devops/labs/jenkins.git` \
...
*   Для того, чтобы Jenkins-контроллер мог связываться с кластером *Kubernetes* и управлять процессом развертывания приложения в нём, они должны друг друга видеть, обычно это подключение реализуется по протоколу ssh по реальными ip-адресам, но если они оба развернуты в docker-контейнерах на локальной машине, то можно воспользоваться возможностями *Docker* и объединить их в общей сети, которую создаёт *Minikube* при запуске. Для этого в файле [docker-compose.yml](https://git.zby.icdc.io/icdc/devops/labs/jenkins/-/blob/dev/docker-compose.yml) для всех запускаемых контейнеров указывается уже существующая сеть *minikube*.  

*   Далее командой \
`docker-compose up -d` \
(флаг `-d` определяет то, что контейнеры будут запущены в фоновом режиме, не блокируя активный терминал) запускаются три контейнера *Jenkins*, *Agent* и *Proxy* согласно файлу [docker-compose.yml](https://git.zby.icdc.io/icdc/devops/labs/jenkins/-/blob/dev/docker-compose.yml), в котором прописан следующее:
...
*Credentials + Credentials Binding, Docker Pipeline, Docker, Pipeline: Basic Steps,  Pipeline: Declarative, Git, GitLab, Generic Webhook Trigger, SSH Build Agents, SSH Credentials, Kubernetes, Kubernetes CLI, Kubernetes Continuous Deploy,Workspace Cleanup* и их зависимости;
...
*   Заходим в [настройки репозитория Gitlab](https://git.zby.icdc.io/-/profile/personal_access_tokens) и создаем токен доступа со Scope = api, сохраняем его себе. Открываем настройки Jenkins > Manage Jenkins > Configure System > Gitlab (Должен появиться после установки соответствующего плагина, если отсутствует -- проверить и установить): \
-ставим галочку в поле "Enable authentication for '/project' end-point", \
-называем "Connection name" подходящим названием, \
-в поле "Gitlab host URL" вводим адрес домена репозитория "https://git.zby.icdc.io/", \
-в поле "Credentials" нажимаем "add", выбираем тип GitLab API token и копируем токен, созданный выше в Gitlab. \
-проверяем соединение Jenkins с репозиторием кнопкой "Test Connection" справа. \
-открываем меню Gitlab > Settings> Webhooks, в поле "URL" копируем адрес URL, выданный при создании нового канала Smee (в данном случае это [https://smee.io/iMncubqOnirnb8yO]), \
-в поле "Secret Token" копируем токен, созданный выше в Jenkins, \
-в меню "triggers" выбираем "push events" и другие на свое усмотрение.
...
*   Согласно файлу [reactapp.yml](https://git.zby.icdc.io/icdc/devops/labs/jenkins/-/blob/dev/reactapp.yml), в kubernetes кластере разворачивается ingress на адрес **reactapp.com**, и для того, чтобы этот адрес корректно отображался в браузере на локальной машине, нужно перенаправить его на адрес minikube, поэтому проверяем ip-адрес minikube командой \
`minikube ip` \
...
-в настройках конвейера проверяем, что в поле "Gitlab Connection"  у нас выбрано название "Connection name" из предыдущего пункта, ставим галочки в следующих полях: \
-"*Build when a change is pushed to GitLab. GitLab webhook URL: http://192.168.49.3:8080/project/node-react-app*" (для автоматической активации конвейера каждый раз, когда на репозитории обновляется или добавляется код), \
-в подменю отмечаем поля "push events" и другие на собственное усмотрение в соответствии с теми ивентами, что были выбраны выше при конфигурировании GitLab, \
-открываем подменю "Advanced" и нажимаем кнопку "Generate" под полем "Secret token", копируем его себе, \
-в *Definition* выбираем из выпадающего меню "*Pipeline script from SCM*", \
-в *SCM* -- "*Git*", \
-в *Repository url* добавляем [url gitlab репозитория](https://git.zby.icdc.io/icdc/devops/labs/jenkins) и добавляем credentials для входа в учетную запись на Gitlab, \
...
в который копируются исходные файлы с репозитория, включая [reactapp.yml](https://git.zby.icdc.io/icdc/devops/labs/jenkins/-/blob/dev/reactapp.yml), \
...
в kubernetes-кластер подается для развертывания (или обновления, если он там уже присутствует) файл конфигурации [reactapp.yml](https://git.zby.icdc.io/icdc/devops/labs/jenkins/-/blob/dev/reactapp.yml). \
...
10.  Последней стадией *post* после окончания работы конвейера, при условии успешного завершения командой `updateGitlabCommitStatus name: 'build', state: 'success'` даём GitLab репозиторию знать, что конвейер выполнен успешно; \
далее, при любом результате окончания выполнения конвейера, будь это успешное развертывание приложения или аварийная остановка из-за ошибки на какой-либо из его стадий, проходит удаление командой `cleanWs()` всей информации , хранящейся в рабочей директории *~/workspace* агента, в частности файлы исходных кодов, которые безусловно должны загружаться заново при каждом изменении на репозитории, чтобы не допускать конфликтов версий и гарантированно каждый раз строить новые версии приложения из нового кода.
...
*  Для демонстрации автоматической работы всей цепочки по сборке приложения по триггеру webhook при каждом обновлении кода на *GitLab* репозитории в терминале на локальной машине находясь в рабочей директории клонированного репозитория jenkins запускаем скрипт командой `./codechange.sh`, который заменяет исходный код приложения на слегка измененную версию из папки "*srcnew*" и отправляет новый коммит на *GitLab* , тем самым запуская всю цепочку выполнения конвейера снова, уже с новым кодом, после выполнения которой (в том числе и обновления kubernetes pods, что займёт несколько минут) нужно обновить страницу в браузере, чтобы увидеть новую версию React приложения. В репозитории для удобства добавлен скрипт `./codechangeback.sh`, который возвращает код в исходное состояние и опять же push'ит его на GitLab, запуская снова Jenkins конвейер и развертывание старой версии приложения. 
